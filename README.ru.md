...
операторы и добавляет аудит конвенций пайплайна ассетов.
...
Якоря по числу нажатий: **D** — **Median** (центр выделения), **DD** —
**Bottom** (XY-медиана + нижняя точка), **DDD** — **Top** (верхняя). Нажатия
подряд, в пределах 0,35 с; четвёртое возвращает Median.
...
| `DD` (подряд, быстро) | — Bottom |
| `DDD` (подряд, быстро) | — Top |
| `Ctrl+D` | Drag Pivot |
| `Ctrl+Alt+D` | Rotate Pivot (объектный режим) |

Pie-меню доступно кнопкой в N-панели.

Все инструменты также живут во вкладке **TOCHKA** N-панели; там же — аудит.

## Решение проблем

**Перестали работать G/R/S (или другие трансформ-хоткеи) во вьюпорте после
обновления TOCHKA?** Версия 1.0.3 записала свои хоткеи в пользовательскую
раскладку и заменила собой системные keymap **Object Mode** и **Mesh**. Лечение
без полного сброса настроек:

1. Обновите TOCHKA до **1.0.4 или новее**
2. Preferences → Keymap → найдите **Object Mode** → нажмите **Restore** (иконка
   рядом с именем keymap)
3. Повторите для **Mesh**
4. Preferences → ☰ → Save Preferences

В версиях 1.0.4+ проблемы нет; в 1.0.0–1.0.2 её тоже не было.
