...
add_table(doc, [
    ...
    ('Origin to Selection', 'D (повторное нажатие - цикл якоря)', 'панель, pie-меню'),
...
p(doc, 'Alt+D остаётся штатной клавишей Blender (Duplicate Linked) - TOCHKA её не занимает. '
       'Pie-меню открывается кнопкой в N-панели.')
...
for b in (
    'Зайдите в режим редактирования колеса, выделите грань на ступице, нажмите D - пивот в центре выделения. Быстрое повторное D переключит якорь: Median → Bottom → Top.',
...
h2(doc, '4.2 Якоря')
add_table(doc, [
    ('Якорь', 'Куда ставится пивот'),
    ('Median', 'центр выделения'),
    ('Bottom', 'XY-медиана выделения, по высоте - нижняя точка'),
    ('Top', 'XY-медиана выделения, по высоте - верхняя точка'),
], [3.6, 13.4])
p(doc, 'Якорь переключается кнопками на панели. Быстрее - двойным нажатием D:')

h2(doc, '4.3 Двойное D - цикл якоря')
p(doc, 'Быстрое повторное нажатие D (в пределах 0,35 секунды) циклит якорь: Median → Bottom → '
       'Top → Median и так далее, каждый раз переставляя пивот по выбранному якорю. Спокойное '
       'одиночное нажатие через паузу просто снова ставит пивот в центр выделения.')
...
add_table(doc, [
    ...
    ('Alt+D не открывает меню TOCHKA', 'с версии 1.1.0 Alt+D - штатный Duplicate Linked', 'pie-меню - кнопкой в N-панели; цикл якоря - двойное D'),
